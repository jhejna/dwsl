...
class VoltronCondEncoder(nn.Module):
    def __init__(
        ...
        self.stack = stack
        self.backbone, self.preprocess_fn = load("v-cond", freeze=True)
        self.extractor = instantiate_extractor(self.backbone, n_latents=1)()
        self.ln = nn.LayerNorm(self.extractor.embed_dim)

        state_dim = observation_space["achieved_goal.state"].shape[0]
        self.output_dim = self.extractor.embed_dim * (self.stack + 1) + (self.stack + int(use_state_goal)) * state_dim
    ...
    def forward(self, obs):
        ...
        h = self.extractor(h)
        h = self.ln(h)
        h = h.view(b, s * self.extractor.embed_dim)
...
class VoltronDualEncoder(nn.Module):
    def __init__(
        self, observation_space: gym.Space, action_space: gym.Space, stack: int = 1, use_state_goal: bool = True
    ):
        super().__init__()
